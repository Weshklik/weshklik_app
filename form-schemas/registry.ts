...
import { realEstateSaleSchema } from './real-estate';
import { phoneSchema } from './phones';
import { carRentalProSchema } from './vehicules/location_voitures.pro';
import { vacationRentalProSchema } from './vacances/location_vacances.pro';
import { TOURISM_ZONE_LABELS as LABELS } from '../data/tourismZones';
import { TOURISM_ZONES_BY_WILAYA as ZONE_MAPPING } from '../data/tourismZoneByLocation';
import { Icons } from '../components/Icons';
import { AdContextType } from '../utils/categoryContextResolver';
...
// --- MAIN RESOLVER ---
export const getFormSchema = (categoryId: string, subCategoryId: string, contextType: AdContextType): FormSchema | null => {
    ...
    // 3. STRICT RULE: NO GENERIC FALLBACK FOR PROS
    // If we are here and context is STORE_PRO, it means no dedicated schema exists.
    if (contextType === 'STORE_PRO') {
        // Return null to trigger the blocking UI in PostAd
        return null;
    }

    // 4. Return Generic Fallback for INDIVIDUALS
